...
function saveUserDetails(){
    var fields = $('detailsFormContent').getElementsByClassName('mui-textfield');

    for(i=0; i<fields.length; i++){
        var field = fields[i];
        var key = field.getElementsByTagName('label')[0].innerText;
        
        var input = field.getElementsByTagName('input');
        if(!input.length){
            input = field.getElementsByTagName('textarea');
        }
        var val = input[0].value;
        
        patient['info'][key] = val;
    }
    
    loadUser_info(patient);
    toggleDisplayClasses_hide('detailEdit');
    toggleDisplayClasses_show('detail');
}

function loadUser_info(obj){
    // Loading info
    var html = ele('div');
    var html2 = ele('div');
    ...
    for(key in obj['info']){
        ...
        //form item
        bigtemp = ele('div');
        bigtemp.className = 'detailEdit mui-textfield hidden';

        temp = ele('label');
        temp.innerText = key;
        bigtemp.appendChild(temp);
        
        if(obj['info'][key].length < 40){
            temp = ele('input');
            temp.setAttribute('type', 'text');
            temp.setAttribute('value', obj['info'][key]);
        }else{
            temp = ele('textarea');
            temp.innerText = obj['info'][key];
        }
        bigtemp.appendChild(temp);

        html2.appendChild(bigtemp);
    }
    
    // add save button
    var temp = ele('button');
    temp.className = 'detailEdit hidden'; 
    temp.innerText = 'Save';
    temp.setAttribute('onclick', 'saveUserDetails();');
    html2.appendChild(temp);

    $('detailsContent').innerHTML = html.innerHTML;
    $('detailsFormContent').innerHTML = html2.innerHTML;
}
